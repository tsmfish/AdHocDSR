...
class AdHocConnect():
    ...
    def send_one_tik_part(self):
        self._send_one_tik_part(source_node=self.nodes_pointers[0], target_node=self.nodes_pointers[1])
        self._send_one_tik_part(source_node=self.nodes_pointers[1], target_node=self.nodes_pointers[0])
    # ---------------------------------------------------------------------------------------------------------------
...
class AdHocNode:
    ...
    def _receive_pack(self, cur_pack):
        ...
        if cur_pack.type == 'rreg':
            if self.check_connect_to(cur_pack.target_node):
                cur_pack.add_node(cur_pack.target_node)
                self._send_rrep(cur_pack)
            else:
                for rreg_pack in self.queue_rereg:
                    if cur_pack.target_node == rreg_pack.target_node:
        ...
        if cur_pack.type == 'rrep':
            if cur_pack.path[0] == self.node_id:
                ...
                if self.node_id == 12:
                    print('Test')
                for rreg_pack in self.queue_rereg:
                    if cur_pack.target_node == rreg_pack.target_node and cur_pack.path[0] == rreg_pack.path[0]:
                        new_pack = deepcopy(cur_pack)
        ...
        if cur_pack.type == 'data':
            if cur_pack.target_node == self.node_id:
                self.model_air.add_report(cur_pack.path[0], cur_pack.path[-1], cur_pack.path[0],
    ...
    def _send_rrep(self, rreg_pack):
        ...
        cur_con = self.get_connect_to(rreg_pack.target_node)
    ...
    def _check_rrep(self, rrep_pack):
        ...
        for pack in self.queue_wait:
            if pack.target_node == rrep_pack.path[-1]:
    ...
    def send_message_to(self, target_node, data_size):
        #  create data pack
        cur_pack = adhoc_pack.DataPack(target_node=target_node, data_size=data_size)
        ...
        for con in self.connect_list:
            cur_pack = adhoc_pack.RRegPack(source_node=self.node_id, target_node=target_node)
            nn = con.get_other_node(self.node_id)
