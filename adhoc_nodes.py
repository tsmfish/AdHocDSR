...
class AdHocConnect:
    ...
    def send_one_tik_part(self):
        self._send_one_tik_part(
            source_node=self.nodes_pointers[0],
            target_node=self.nodes_pointers[1],
        )
        self._send_one_tik_part(
            source_node=self.nodes_pointers[1],
            target_node=self.nodes_pointers[0],
        )
        ...
        for pack in source_node.queue_to_send:
            if pack.get_next_hop() == target_node.node_id:
                ...
                if pack.size_was_sent >= pack.get_size():
                    source_node.model_air.register_route_discovery_packet_sent(pack)
...
# --------------------------------------------------------------------------------------------------------------
class AdHocNode:
    def __init__(self, node_id: int, y: int, x: int, model_air):
    ...
    def _receive_pack(self, cur_pack: BasePack):
        ...
        if isinstance(cur_pack, RReqPack):
            if self.check_connect_to(cur_pack.destination_node_id):
                if cur_pack.decrease_ttl_for_forwarding():
                    cur_pack.add_node(cur_pack.destination_node_id)
                    self._send_rrep(cur_pack)
                else:
                    self._register_ttl_expired(cur_pack, "rreg")
            else:
                if cur_pack.decrease_ttl_for_forwarding():
                    for rreq_pack in self.queue_rereg:
                        if cur_pack.destination_node_id == rreq_pack.destination_node_id:
        ...
        if isinstance(cur_pack, RRepPack):
            if cur_pack.path[0] == self.node_id:
                ...
                if cur_pack.decrease_ttl_for_forwarding():
                    ...
                    self._register_ttl_expired(cur_pack, "rrep")
        ...
        if isinstance(cur_pack, DataPack):
            if cur_pack.destination_node_id == self.node_id:
                ...
                if cur_pack.decrease_ttl_for_forwarding():
                    ...
                    self._register_ttl_expired(cur_pack, "data")

    # -----------------------------
    def _register_ttl_expired(self, cur_pack: BasePack, pack_type: str):
        # print(
        #     f"[{self.model_air.get_current_time()}] Сповіщення: "
        #     f"Пакет {cur_pack.__class__.__name__} від вузла "
        #     f"{cur_pack.source_node_id} до вузла {cur_pack.destination_node_id} "
        #     f"не може бути доставлений "
        #     f"(відкинуто на вузлі {self.node_id} через закінчення TTL)."
        # )
        self.model_air.add_lost(
            cur_pack.source_node_id,
            cur_pack.destination_node_id,
            cur_pack.path,
            f"{self.node_id}:TTL_EXPIRED",
            pack_type,
        )
    ...
    def _check_data_send(self):
        ...
        for pack in self.queue_wait:
            if len(pack.rrep_list) > 0:
                ...
                if self.model_air.get_current_time() > (
                    min(all_time) + RREP_WAIT_TICs
                ):
