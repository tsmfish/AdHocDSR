...
class DSR_node:
    ...
    # -----------------------------
    def check_connect_to(self, id: int):
    ...
    # -----------------------------
    def get_connect_to(self, id: int) -> Connect | None:
        for con in self.connect_list:
            if id in con.nodes_id:
                return con
        return None
    ...
    # -----------------------------
    def send_rreg(self, node_list: list["DSR_node"], destination_node: "DSR_node"):
        rez = []
        final = []
        for con in self.connect_list:
...
class nets_mesh:
    ...
    def create_net(self):
        ...
        for xx in range(nodes_count):
            ...
            while len(self.nodes_list[xx].connect_list) < con_count and loop_calc < 30:
                ...
                loop_calc += 1
                if con_nod != xx and (
                    not self.nodes_list[con_nod].check_connect_to(xx)
                ):
                    if (
                        len(self.nodes_list[con_nod].connect_list)
                        >= self._max_connect_count
                    ):
                        continue
                    new_con = Connect()
    ...
    def create_path(self, from_node, to_node):
        ...
        while len(rreg) > 0:
            ...
            if len(cur_rreg) < len(self.nodes_list) // 2:
                cur_rreg, final_rreg = self.nodes_list[cur_rreg[-1]].send_rreg(
                    cur_rreg, to_node
                )
    ...
    def save_compare_results(self, file_name):
        ...
        for pack_size in [300, 500, 700, 1000, 1200, 1500]:
            ...
            row = [
                len(self.nodes_list),
                len(self.connect_list),
                pack_size,
                target_index,
                dsr_index,
                forecast_index,
                len(path[target_index]),
                len(path[dsr_index]),
                len(path[forecast_index]),
                true_target[target_index],
                true_target[dsr_index],
                true_target[forecast_index],
                pred[forecast_index],
            ]
            f = open(file_name, "a+", newline="\n")
            wr = csv.writer(f, quoting=csv.QUOTE_ALL)
...
if __name__ == "__main__":
    for xx in range(200):
        ...
        cur_net.save_compare_results(
            r"d:\Sychov\HNUPS\Projects\DSR_data\rez\snr_rez_full_v2.csv"
        )
